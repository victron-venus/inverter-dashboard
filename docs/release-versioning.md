...
- `VERSION`
- `src/inverter_dashboard/VERSION`

The Python distribution metadata and its own lock entry intentionally remain `0.0.0`; runtime identity comes from both VERSION files. The existing frozen application smoke test verifies the actual packaged runtime version.
